...
export default async function AdminDashboardPage() {
  ...
  const [users, loans, pending, cashouts, installments, profileReviews, depositRequests, applicationPayments, approved] = await Promise.all([
    ...
    supabase.from('loan_application_payments').select('*', { count:'exact', head:true }).eq('status', 'submitted'),
  ...
  return <AdminDashboardView stats={{ users:users.count ?? 0, loans:loans.count ?? 0, pending:pending.count ?? 0, cashouts:cashouts.count ?? 0, installments:installments.count ?? 0, profileReviews:profileReviews.count ?? 0, depositRequests:depositRequests.count ?? 0, applicationPayments:applicationPayments.count ?? 0, approvedAmount:total }}/>;
}
