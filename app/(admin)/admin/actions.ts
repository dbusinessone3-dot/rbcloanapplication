...
export async function reviewLoanAction(formData: FormData) {
  ...
  revalidatePath('/admin/loans');revalidatePath(`/admin/loans/${loanId}`);revalidatePath('/dashboard');
}
...
export async function reviewLoanApplicationPaymentAction(formData: FormData) {
  ...
  if (decision === 'approve') {
    await supabase.from('loan_application_payments').update({
      status: 'verified',
      verified_by: admin.id,
    ...
    await supabase.from('app_notifications').insert({
      ...
      type: 'loan_application_payment_verified',
      title: 'Application payment approved',
      message: note || 'Your required savings payment was approved. You can now submit the final loan application.',
      action_url: '/loan/apply',
    });
  } else if (decision === 'reject') {
    ...
    await supabase.from('app_notifications').insert({
      user_id: payment.user_id,
      type: 'loan_application_payment_rejected',
      title: 'Application payment needs attention',
      message: reason,
      action_url: '/loan/apply',
    });
  } else {
  ...
  await supabase.from('admin_activity_logs').insert({
    admin_user_id: admin.id,
    action: `loan_application_payment_${decision}`,
    subject_type: 'loan_application_payment',
    subject_id: paymentId,
    description: note || null,
    metadata: { user_id: payment.user_id, amount: payment.expected_amount },
  });
