...
export async function submitLoanApplicationPaymentAction(_: ActionState, formData: FormData): Promise<ActionState> {
  ...
  try {
    uploadedPath = `${user.id}/loan-application-payments/${safeName(screenshot)}`;
    await uploadFile(supabase, 'payment-proofs', uploadedPath, screenshot);

    const { error } = await supabase.rpc('submit_loan_application_payment_v2', {
      p_requested_amount: requestedAmount,
      p_payment_method: paymentMethod,
      p_receiver_reference: receiverReference || null,
      p_transaction_id: transactionId,
      p_payment_screenshot_path: uploadedPath,
    });

    if (error) {
      await supabase.storage.from('payment-proofs').remove([uploadedPath]);
      return fail(isApplicationPaymentSchemaError(error) ? APPLICATION_PAYMENT_SETUP_MESSAGE : error.message);
    }
    ...
    return ok('Application fee submitted successfully. Admin has been notified and will review your payment.');
  } catch (error) {
    if (uploadedPath) await supabase.storage.from('payment-proofs').remove([uploadedPath]);
    return fail(error instanceof Error ? error.message : 'Unable to submit the application fee.');
  }
}
...
export async function applyLoanAction(_: ActionState, formData: FormData): Promise<ActionState> {
  ...
  if (verifiedPaymentError && isApplicationPaymentSchemaError(verifiedPaymentError)) {
    return fail(APPLICATION_PAYMENT_SETUP_MESSAGE);
  }
  ...
  await supabase.from('app_notifications').insert({
    ...
    message: `${applicationNo} has been submitted for admin review.`, action_url: `/my-loans/${loan.id}`,
  });
  revalidatePath('/dashboard');
  revalidatePath('/my-loans');
  revalidatePath(`/my-loans/${loan.id}`);
  revalidatePath('/notifications');
