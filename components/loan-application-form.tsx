...
import { Building2, CheckCircle2, Clock3, FileUp, ReceiptText, Send, ShieldCheck, Smartphone, UserRound, XCircle } from 'lucide-react';
    ...
    <div className="content-grid application-layout">
      <div className="application-main-column">
        ...
        <section className="form-card mobile-form-shell application-payment-step">
          ...
          {paymentWaiting ? <div className="application-payment-status waiting"><Clock3 size={24}/><div><strong>{t('waitingAdmin')}</strong><p>{t('applicationPaymentWaiting')}</p></div></div> : null}
        ...
        <div className={`final-application-submit ${paymentReady ? 'ready' : 'locked'}`}>
          ...
          <button className="primary-btn" type="submit" form="loan-application-main" disabled={!paymentReady} aria-disabled={!paymentReady}>{t('submitApplication')}</button>
        </div>
      </div>
