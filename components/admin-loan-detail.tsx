    ...
    <div className="admin-page-stack">
      ...
      <div className="content-grid admin-loan-detail-grid">
        ...
        <aside className="form-card admin-review-decision-card">
          ...
          {isApproved ? <div className="admin-final-decision approved"><CheckCircle2 size={34} /><div><strong>Application approved</strong><p>The review is complete. The user has been notified and can view the approved application from My Loans.</p></div><StatusBadge status={loan.status} /></div> : null}
          {isRejected ? <div className="admin-final-decision rejected"><XCircle size={34} /><div><strong>Application rejected</strong><p>{loan.rejection_reason || 'The application was rejected.'}</p></div><StatusBadge status={loan.status} /></div> : null}
          {canReview ? <>
            ...
            <form action={reviewLoanAction} className="form-grid">
              ...
              <div className="admin-preview-card"><div className="admin-preview-head"><Calculator size={19} /><span>Approval calculation</span></div><div className="summary-list">
              ...
              <div className="admin-decision-actions">
                <button className="secondary-btn" name="decision" value="review">{loan.status === 'under_review' ? 'Reviewing' : t('underReview')}</button>
